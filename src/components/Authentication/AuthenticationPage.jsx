...
const AuthenticationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      register(formData); // already handles duplicate check internally
      setLoading(true);
      // auto-login karwa de ya redirect kar de
      const success = login(formData.email, formData.password);
      if (success) {
        window.location.href = "/";
      } else {
        setErrors({ general: "Something went wrong while registering." });
        setLoading(false);
      }
    }
  };
